...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, Compass, HeartHandshake, MapPin, Search, ShieldCheck, Sparkles, Users, WandSparkles } from 'lucide-react';
...
import { auth, completeOnboarding, intents } from '../services/repo';
...
type Draft = {
  userId: string; step: number; adult: boolean; name: string; city: string; roles: Role[];
...
function initialDraft(me: User): Draft {
  const empty: Draft = { userId: me.id, step: 0, adult: false, name: me.name, city: me.city, roles: me.roles.slice(0, 3), goals: [], interests: [], skills: [], needs: [], availability: me.availability, experience: me.experience, text: '', preview: null };
  try {
...
export function Welcome() {
  ...
  return <main className="flex min-h-full flex-col px-7 pb-7 pt-8">
    ...
    <div className="space-y-3">
      ...
      <button type="button" className="btn btn-secondary w-full" onClick={enter} disabled={busy}>Continue with Demo</button>
      <p className="text-center text-xs text-[var(--text-2)]">Already here? <button type="button" onClick={enter} disabled={busy} className="min-h-11 px-2 font-semibold text-[var(--accent-text)]">Sign in</button></p>
      <p className="text-center text-[10px] leading-relaxed text-[var(--text-2)]">18+ community · Local demo, no password needed.<br />All options return to your saved demo on this device.</p>
    </div>
  </main>;
}
...
export function Onboarding() {
  ...
  const createdIntent = useRef<string | null>(null);
  ...
  function next(event: FormEvent) {
    ...
    if (step === 0) { const message = profileError(); if (message) { setError(message); return; } }
    ...
    try {
      // The ref avoids a duplicate intent if a later profile save needs a retry.
      if (!createdIntent.current) createdIntent.current = intents.create(draft.text, { interpretation: preview }).id;
      completeOnboarding({ name: draft.name.trim(), city: draft.city.trim(), roles: draft.roles, interests: draft.interests, skills: draft.skills, needs: draft.needs, availability: draft.availability, experience: draft.experience, currently: preview.goal, headline: draft.roles.map(pretty).join(' · ') });
