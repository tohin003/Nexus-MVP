...
test('photo posts appear in own Home feed and persist; suggestions scroll horizontally', async ({ page }) => {
  ...
  await page.getByLabel('Attach post photo', { exact: true }).setInputFiles(photo);
  await expect(page.getByAltText('Attach post photo preview')).toHaveAttribute('src', /^data:image\/jpeg;base64,/);
  await page.getByRole('button', { name: 'Publish post', exact: true }).click();
  await page.getByRole('button', { name: 'Go to feed' }).click();
  await expect(page.getByAltText('Photo for A photo of progress')).toBeVisible();
  await page.reload();
  await expect(page.getByAltText('Photo for A photo of progress')).toHaveAttribute('src', /^data:image\/jpeg;base64,/);
  const rail = page.getByRole('region', { name: 'Suggested people' });
